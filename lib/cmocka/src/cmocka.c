 ...
 * Copyright 2021      Leuenberger Niklaus <leuen4@bfh.ch>
 *
...
void _test_free(void* const ptr, const char* file, const int line) {
    ...
    /* Check the guard blocks. */
    {
        ...
        for (unsigned int i = 0; i < ARRAY_SIZE(guards); i++) {
            unsigned int j;
                         ...
                         const char *error_message)
{
    ...
    case PRINTF_TEST_FAILURE:
        ...
        if (error_message != NULL) {
            ...
            msg = strdup(error_message); // cppcheck-suppress [strdupCalled]
            if (msg == NULL) {
                return;
            }
            p = msg;
...
#ifndef _WIN32
        for (unsigned int i = 0; i < ARRAY_SIZE(exception_signals); i++) {
